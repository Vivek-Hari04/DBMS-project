...
type CreateJobRequest struct {
    ...
    JobType      string   `json:"job_type"` // "temporary" | "permanent"
}
...
func (h *JobHandler) CreateJob(c *gin.Context) {
    ...
    var userType string
    if err := h.DB.QueryRow(context.Background(), `SELECT user_type FROM users WHERE id = $1`, empID).Scan(&userType); err != nil {
    ...
    // Shopkeeper MUST supply a valid shop they own
    if userType == "shopkeeper" {
        if req.ShopID == nil {
            c.JSON(http.StatusBadRequest, gin.H{"error": "shop_id is required for shopkeeper accounts"})
            return
        }
        var shopOwnerID int
        if err := h.DB.QueryRow(context.Background(),
            `SELECT owner_id FROM shops WHERE id = $1`, *req.ShopID,
        ).Scan(&shopOwnerID); err != nil {
            c.JSON(http.StatusBadRequest, gin.H{"error": "Shop not found"})
            return
        }
        if shopOwnerID != empID {
    ...
    err := h.DB.QueryRow(context.Background(), `
        INSERT INTO jobs (
            ...
            contact_phone, contact_email, expires_at, shop_id, job_type
        ...
        req.ContactPhone, req.ContactEmail, expiresAt, req.ShopID, jobType,
    ).Scan(&jobID)
    if err != nil {
    ...
    h.DB.Exec(context.Background(), `
        ...
        FROM users WHERE user_type IN ('worker', 'handyman')
    `, req.Title, req.Location)

    c.JSON(http.StatusCreated, gin.H{"message": "Job created successfully", "job_id": jobID})
}

// buildJobMap converts raw job query row into the standard response map
func buildJobMap(id, employerID int, title, description, location, status, employerName string,
    jobType string,
    shopID sql.NullInt64, shopName sql.NullString,
    shopCategory, shopDescription, shopLocation sql.NullString,
    categoryID sql.NullInt64, categoryName sql.NullString,
    salaryMin, salaryMax sql.NullFloat64,
    duration, requirements, contactPhone, contactEmail sql.NullString,
    expiresAt, createdAt time.Time, isActive bool,
    hiredWorkerID sql.NullInt64, averageRating float64,
    ...
    if shopID.Valid {
        job["shop_id"] = shopID.Int64
        name := ""
        if shopName.Valid { name = shopName.String }
        pBy := map[string]interface{}{
            "type":    "shop",
            "name":    name,
            "shop_id": shopID.Int64,
        }
        if shopCategory.Valid { pBy["category"] = shopCategory.String }
        if shopDescription.Valid { pBy["description"] = shopDescription.String }
        if shopLocation.Valid { pBy["location"] = shopLocation.String }
        job["posted_by"] = pBy
    } else {
        job["posted_by"] = map[string]interface{}{
            "type": "user",
            "name": employerName,
        }
    }
    if hiredWorkerID.Valid { job["hired_worker_id"] = hiredWorkerID.Int64 }
    if categoryID.Valid { job["category_id"] = categoryID.Int64 }
    if categoryName.Valid { job["category_name"] = categoryName.String }
    if salaryMin.Valid { job["salary_min"] = salaryMin.Float64 }
    if salaryMax.Valid { job["salary_max"] = salaryMax.Float64 }
    if duration.Valid { job["duration"] = duration.String }
    if requirements.Valid { job["requirements"] = requirements.String }
    if contactPhone.Valid { job["contact_phone"] = contactPhone.String }
    if contactEmail.Valid { job["contact_email"] = contactEmail.String }
    return job
}

const jobSelectCols = `
    j.id, j.employer_id, j.title, j.description, j.category_id, j.location,
    ...
    COALESCE(j.job_type, 'temporary') as job_type,
    j.shop_id,
    s.name as shop_name,
...
const jobSelectJoins = `
    FROM jobs j
    JOIN users u ON j.employer_id = u.id
    LEFT JOIN categories c ON j.category_id = c.id
    LEFT JOIN shops s ON j.shop_id = s.id`

func scanJobRow(rows interface {
    Scan(dest ...interface{}) error
}) (map[string]interface{}, error) {
    var (
        id, employerID int
        title, description, location, status, employerName, jobType string
        categoryID, shopID, hiredWorkerID sql.NullInt64
        salaryMin, salaryMax sql.NullFloat64
        duration, requirements, contactPhone, contactEmail, categoryName, shopName sql.NullString
        shopCategory, shopDescription, shopLocation sql.NullString
    ...
    if err := rows.Scan(
        ...
        &hiredWorkerID, &employerName, &categoryName, &averageRating,
        &jobType, &shopID, &shopName, &shopCategory, &shopDescription, &shopLocation,
    ); err != nil {
        return nil, err
    }
    return buildJobMap(id, employerID, title, description, location, status, employerName,
        jobType, shopID, shopName, shopCategory, shopDescription, shopLocation, categoryID, categoryName,
        salaryMin, salaryMax, duration, requirements, contactPhone, contactEmail,
        expiresAt, createdAt, isActive, hiredWorkerID, averageRating), nil
}

// Get all active jobs
func (h *JobHandler) GetJobs(c *gin.Context) {
    query := `SELECT ` + jobSelectCols + jobSelectJoins + `
...
// Search active jobs by keyword and/or location
func (h *JobHandler) SearchJobs(c *gin.Context) {
    q := strings.TrimSpace(c.Query("q"))
    loc := strings.TrimSpace(c.Query("location"))

    baseQuery := `SELECT ` + jobSelectCols + jobSelectJoins +
        ` WHERE j.is_active = true AND (j.is_private = false OR j.is_private IS NULL)`
...
func (h *JobHandler) GetJob(c *gin.Context) {
    ...
    row := h.DB.QueryRow(context.Background(), query, jobID)
...
func (h *JobHandler) GetMyJobs(c *gin.Context) {
    ...
    query := `
        SELECT j.id, j.employer_id, j.title, j.description, j.category_id, j.location,
               j.salary_min, j.salary_max, j.duration, j.requirements,
               j.contact_phone, j.contact_email, j.expires_at, j.status, j.is_active, j.created_at,
               j.hired_worker_id, j.is_private, j.assigned_worker_id, j.shop_id,
               u.full_name as employer_name,
               c.name as category_name,
               s.name as shop_name,
               COALESCE((SELECT AVG(rating) FROM ratings WHERE reviewee_id = j.employer_id), 0) as average_rating
    ...
    for rows.Next() {
        var (
            ...
            isPrivate bool
            assignedWorkerID, shopID sql.NullInt64
        ...
        err := rows.Scan(
            ...
            &salaryMin, &salaryMax, &duration, &requirements,
            &contactPhone, &contactEmail, &expiresAt, &status, &isActive, &createdAt,
            &hiredWorkerID, &isPrivate, &assignedWorkerID, &shopID,
            &employerName, &categoryName, &shopName, &averageRating,
        )
        if err != nil {
            continue
        }
        ...
        if shopName.Valid { job["shop_name"] = shopName.String }
        if shopID.Valid { job["shop_id"] = shopID.Int64 }
